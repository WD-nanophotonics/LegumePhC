...
class StudioApp(tk.Tk):
    ...
    def _build_geometry_tab(self) -> None:
        ...
        ttk.Label(controls, text="Motifs (name / kind / radius / center / epsilon)").grid(row=17, column=0, columnspan=2, sticky="w", padx=4, pady=(8, 2))
        self.motif_tree = ttk.Treeview(controls, columns=("name", "kind", "radius", "center", "epsilon", "sides", "angle"), show="headings", height=4)
        for column, heading, width in (("name", "Name", 90), ("kind", "Kind", 65), ("radius", "R", 45), ("center", "Center", 100), ("epsilon", "Epsilon", 65), ("sides", "Sides", 45), ("angle", "Angle", 50)):
            self.motif_tree.heading(column, text=heading)
            self.motif_tree.column(column, width=width, stretch=False)
        self.motif_tree.grid(row=18, column=0, columnspan=2, padx=4, pady=2)
        motif_buttons = ttk.Frame(controls)
        motif_buttons.grid(row=19, column=0, columnspan=2, sticky="ew", padx=4, pady=2)
        ...
        self.preview_host = ttk.Frame(self.geometry_side)
        self.preview_host.pack(fill="both", expand=True, padx=6, pady=6)
    ...
    def _build_calculation_tab(self) -> None:
        ...
        self.calc_status.grid(row=15, column=0, columnspan=2, sticky="w", padx=4)
        self._label_entry(controls, 16, "Berry source result", "source_result", 28)
        self._label_entry(controls, 17, "Response weights", "response_weights", 28)
    ...
    def _build_style_tab(self) -> None:
        ...
        ttk.Label(controls, text="Band style").grid(row=12, column=0, sticky="w", padx=4, pady=2)
        band_style = ttk.Combobox(controls, textvariable=self._var("band_style"), values=("line", "scatter", "cycle"), state="readonly", width=12)
        band_style.grid(row=12, column=1, sticky="w", padx=4, pady=2)
        self._style_widgets["band_style"] = [controls.grid_slaves(row=12, column=0)[0], band_style]
        ttk.Label(controls, text="Field quantity").grid(row=13, column=0, sticky="w", padx=4, pady=2)
        ...
        field_quantity.grid(row=13, column=1, sticky="w", padx=4, pady=2)
        self._style_widgets["field_quantity"] = [controls.grid_slaves(row=13, column=0)[0], field_quantity]
        self._style_bool(controls, 14, "Grid", "grid")
        self._style_bool(controls, 15, "Legend", "legend")
        self._style_bool(controls, 16, "Berry samples", "berry_coloring")
        self._style_bool(controls, 17, "Berry interpolation", "berry_interpolation")
        self._style_bool(controls, 18, "Colorbar", "colorbar")
        self._label_entry(controls, 19, "Berry vmin", "berry_vmin", 12)
        self._label_entry(controls, 20, "Berry vmax", "berry_vmax", 12)
        ...
        ttk.Button(controls, text="Export current plot…", command=self._export).grid(row=21, column=0, columnspan=2, sticky="ew", padx=4, pady=6)
        operation_var = self._vars.get("operation")
    ...
    def _update_style_visibility(self, operation: str | None = None) -> None:
        ...
        specific = {
            "solve_bands": {"band_style", "linewidth", "marker_size"}, "band_structure": {"band_style", "linewidth", "marker_size"},
    ...
    def _case_from_controls(self) -> dict[str, Any]:
        ...
        for item in self.motif_tree.get_children():
            name, kind, radius, center, epsilon, sides, angle = self.motif_tree.item(item, "values")
            motif = {"name": name, "kind": kind, "radius": float(radius), "center": json.loads(center), "epsilon": float(epsilon), "sides": int(float(sides or 8)), "angle_degrees": float(angle or 0.0)}
            if kind == "polygon":
                motif["sides"] = max(3, motif["sides"])
            motifs.append(motif)
        if motifs:
            case["geometry"]["motifs"] = motifs
            case["geometry"]["kind"] = motifs[0]["kind"]
            case["geometry"]["radius"] = motifs[0]["radius"]
            case["geometry"]["center"] = motifs[0]["center"]
    ...
    def _populate_motifs(self) -> None:
        ...
        for index, motif in enumerate(motifs):
            self.motif_tree.insert("", "end", iid=f"motif-{index + 1}", values=(motif.get("name", f"motif-{index + 1}"), motif.get("kind", "circle"), motif.get("radius", 0.2), json.dumps(motif.get("center", [0.5, 0.5])), motif.get("epsilon", geometry.get("epsilon_inclusion", 1.0)), motif.get("sides", 8), motif.get("angle_degrees", 0.0)))

    def _add_motif(self) -> None:
        index = len(self.motif_tree.get_children()) + 1
        self.motif_tree.insert("", "end", iid=f"motif-{index}", values=(f"motif-{index}", "circle", 0.2, "[0.5, 0.5]", 1.0, 8, 0.0))
        self._mark_pending()
    ...
    def _edit_motif(self) -> None:
        ...
        for index, prompt in ((0, "Motif name"), (1, "Kind (circle/polygon)"), (2, "Radius"), (3, "Center [x, y]"), (4, "Epsilon"), (5, "Polygon sides"), (6, "Angle degrees")):
            answer = simpledialog.askstring("Edit motif", prompt, initialvalue=str(values[index]), parent=self)
            if answer is None:
                return
            values[index] = answer
    ...
    def _refresh_preview(self) -> None:
        try:
            ...
            self.preview_toolbar.pack(side="bottom", fill="x")
